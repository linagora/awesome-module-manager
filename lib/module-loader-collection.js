...
var AwesomeModuleWrapper = require('./module-wrapper');
...
AwesomeModuleLoaderCollection.prototype._loadRequiredNamedDependencies = function(module, moduleContext) {
  ...
  this.log_insane('AwesomeModuleLoaderCollection._loadRequiredNamedDependencies(', module.name, ') got ', dependencies.length, 'dependencies to fetch');
...
AwesomeModuleLoaderCollection.prototype._loadOptionalNamedDependencies = function(module, moduleContext) {
  ...
  this.log_insane('AwesomeModuleLoaderCollection._loadOptionalNamedDependencies(', module.name, ') got ', dependencies.length, 'dependencies to fetch');
  var d = q.defer();
  q.all(
    dependencies.map(
      function(dep) {
          ...
          .then(function(module) {
            ...
            this.log_insane('AwesomeModuleLoaderCollection._loadOptionalNamedDependencies(', dep.name, ') failed, silently ignoring');
            return q(true);
          }.bind(this));
      }.bind(this)
    )
...
AwesomeModuleLoaderCollection.prototype._getRequiredAbilityDependencies = function(module) {
  ...
  this.log_insane('AwesomeModuleLoaderCollection._getRequiredAbilityDependencies(' + module.name + '), ' + abdependencies.length + ' dependencies');
  var missingAbDependencies = abdependencies.filter(function(depres) { return depres.found === false; });
  this.log_insane('AwesomeModuleLoaderCollection._getRequiredAbilityDependencies(', module.name, '), ', missingAbDependencies.length, 'missing dependencies');
...
AwesomeModuleLoaderCollection.prototype.loadModuleAndDependencies = function(name, onModule, moduleContext) {
  ...
  this.log_insane('AwesomePluginManager loadModuleAndDependencies starts(', name, ')');
  ...
  .then(function(mod) {
    module = mod;
    self.log_insane('AwesomeModuleLoaderCollection.loadModuleAndDependencies(', name, ') dependency call');
    return q(module);
  })
  .then(this._loadDependencies(moduleContext))
  .then(function(dependencies) {
    this.log_insane('AwesomeModuleLoaderCollection.loadModuleAndDependencies[' + name + '] all dependencies resolved, sending back the module');
  ...
  function _loadOne() {
    ...
    loader.load(name, function(err, resp) {
      if (err) {
        ...
        var wrappedModule = new AwesomeModuleWrapper(resp, loader.trusted);
        return deferred.resolve(wrappedModule);
      }
      _loadOne();
    });
  }
...
require('./logger-helper-mixin')(AwesomeModuleLoaderCollection);
